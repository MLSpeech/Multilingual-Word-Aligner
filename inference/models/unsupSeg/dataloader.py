...
def save_cnn_presentation(orig_file_name, output_dir, tensor):
    file_name = os.path.basename(orig_file_name).split('.')[0]  # Extract base name without extension
    output_file_name = f"{file_name}_UnsupSeg_cnn.pkl"
    output_file_path = os.path.join(output_dir, output_file_name)
    with open(output_file_path, 'wb') as f:
        pickle.dump(tensor, f, protocol=pickle.HIGHEST_PROTOCOL)

def save_score_presentation(orig_file_name, output_dir, tensor):
    # Extract base name without extension
    file_name = os.path.basename(orig_file_name).split('.')[0]
    
    # Create output file name
    output_file_name = f"{file_name}_UnsupSeg_score.pkl"
    output_file_path = os.path.join(output_dir, output_file_name)
    
    # Convert tensor to NumPy array, move to CPU, and detach from computation graph
    tensor_data = tensor.cpu().detach().numpy()
    
    # Save only the tensor data to the pickle file
    with open(output_file_path, 'wb') as f:
        pickle.dump(tensor_data, f, protocol=pickle.HIGHEST_PROTOCOL)

def save_model_train_cnn_presentation(orig_file_name, output_dir, tensor):
    # Extract base name without extension
    file_name = os.path.basename(orig_file_name).split('.')[0]
    
    # Create output file name
    output_file_name = f"{file_name}_UnsupSeg_model_train_cnn.pkl"
    output_file_path = os.path.join(output_dir, output_file_name)
    
    # Convert tensor to NumPy array, move to CPU, and detach from computation graph
    tensor_data = tensor.squeeze(0).cpu().detach().numpy()
